SELECT DISTINCT cc.id, STRING_AGG(DISTINCT mpg.measuredpart 
        ...
        ||CASE WHEN (dim.dimension IS NOT NULL AND dim.dimension <>0 AND dim.measurementunit IS NOT NULL AND dim.measurementunit <>'') THEN ' (' || dim.measurementunit || ')' ELSE '' END, '␥') AS "objdimensions"
