SELECT DISTINCT cc.id, STRING_AGG(DISTINCT REGEXP_REPLACE(apg.assocpeople, '^.*\)''(.*)''$', '\1'),'␥') AS "objassoccult_ss", STRING_AGG(DISTINCT utils.culture_hierarchy.culture_hierarchy, '␥') AS "objculturetree_ss"
