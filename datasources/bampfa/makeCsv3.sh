...
time psql -R"@@" -A -U reporter -d "host=$HOST.cspace.berkeley.edu dbname=bampfa_domain password=csR2p4rt2r" -f metadata.sql -o d1.csv
...
time psql -R"@@" -A -U reporter -d "host=$HOST.cspace.berkeley.edu dbname=bampfa_domain password=csR2p4rt2r" -f media.sql -o m1.csv
time perl -pe 's/[\r\n]/ /g;s/\@\@/\n/g' m1.csv > media.csv 
# make the header
