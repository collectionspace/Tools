...
def doSingleObjectSearch(form, config, displaytype=''):
    ...
    if updateType == 'barcodeprint':
        try:
            obj = cswaDB.getobjinfo(form.getvalue('ob.objectnumber'), config)
        except:
            raise
            handleTimeout('search', form)
        print """
...
def doEnumerateObjects(form,config):
    ...
    if rowcount == 0 and not(updatetype=='barcodeprint'):
        print '*****************************DEBUG*****************************'
...
def doBarCodes(form,config):
    ...
    if form.getvalue('ob.objectnumber') != '':
        ...
        if action=='Create Labels for Objects':
            labelFilename = writeCommanderFile(obj[3],form.getvalue("printer"),'objectLabels','objects',obj,config)
