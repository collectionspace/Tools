...
def doEnumerateObjects(form, config):
    ...
    try:
        locationList = cswaDB.getloclist('range', form.getvalue("lo.location1"), form.getvalue("lo.location2"), num2ret,
                                         config)
    except:
        raise
...
def doUpdateKeyinfo(form, config):
    ...
    for row, csid in enumerate(CSIDs):
        ...
        if not refNames2find.has_key(form.getvalue('cp.' + index)):
            refNames2find[form.getvalue('cp.' + index)] = cswaDB.getrefname('places_common',
                                                                            form.getvalue('cp.' + index), config)
        if not refNames2find.has_key(form.getvalue('cg.' + index)):
            refNames2find[form.getvalue('cg.' + index)] = cswaDB.getrefname('concepts_common',
                                                                            form.getvalue('cg.' + index), config)
        if not refNames2find.has_key(form.getvalue('fc.' + index)):
            refNames2find[form.getvalue('fc.' + index)] = cswaDB.getrefname('concepts_common',
                                                                            form.getvalue('fc.' + index), config)
...
def doPackingList(form, config):
    ...
    print """<tr><td align="center" colspan="6"><hr><td></tr>"""
    print """<tr><td align="center" colspan="6">Packing list completed. %s objects, %s locations, %s including crates</td></tr>""" % (
...
def doLocationList(form, config):
    ...
    print """<tr><td align="center">Location Report completed. %s objects of %s displayed</td></tr>""" % (
    len(accessions), len(objects))
    print "\n</table><hr/>"
...
def doBarCodes(form, config):
    ...
    rows.reverse()
    if action == "Create Labels for Locations Only":
        labelFilename = writeCommanderFile('locations', form.getvalue("printer"), 'locationLabels', 'locations', rows,
                                           config)
        ...
        for r in rows:
            ...
            labelFilename = writeCommanderFile(r[0], form.getvalue("printer"), 'objectLabels', 'objects', objects,
                                               config)
...
def writeCommanderFile(location, printerDir, dataType, filenameinfo, data, config):
    ...
    logFile = config.get('files', 'cmdrfmtstring') % (config.get('files', 'cmdrfileprefix'), dataType, printerDir, slug,
...
def formatRow(result, form, config):
    ...
    if result['rowtype'] == 'subheader':
        ...
        link, rr[3], rr[5], rr[3], rr[8], rr[7], rr[6], rr[3], rr[14], rr[3], rr[8], rr[7], rr[6], rr[3], rr[14], rr[3])
...
<td><input class="xspan" type="text" size="26" name="fc.%s" value="%s"></td>
        ...
        link, rr[3], rr[8], rr[4], rr[8], rr[5], rr[8], rr[3], rr[8], rr[8], rr[8], rr[6], rr[8], rr[7], rr[8], rr[9])
...
def selectWebapp():
    webapps = {
    'pahma': ['sysinv', 'keyinfo', 'packlist', 'move', 'upload', 'barcodeprint', 'collectionStats', 'objectInfo'],
