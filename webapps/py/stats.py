...
def progress(location):
    ...
    cumulative = 0
    for d in range(1, 12):
        query = "something like: select count(*) from table where location = '%s' and date = '2012-%02d'" % (
        location, d)
        #execute(query)
...
if __name__ == "__main__":
    ...
    print '''<img src="//chart.googleapis.com/chart?chxt=y&chs=300x220&cht=lc&chco=FF0000,008000&chd=t:%s|%s&chdl=kroeber|regatta&&chds=a&chls=1|1" width="300" height="220" alt="" />''' % (
    ','.join(counts[0]), ','.join(counts[1]))
    print "<html>"
