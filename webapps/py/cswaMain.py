...
# special case: if only one location in range, jump to enumerate
    ...
    if updateType == 'packinglist':
        ...
        doComplexSearch(form, config, 'select')
    elif updateType == 'advsearch':
        doComplexSearch(form, config, 'select')
    elif updateType == 'locreport':
        doLocationList(form, config)
    elif updateType == 'inventory':
        doLocationSearch(form, config, 'list')
    elif updateType == 'keyinfo':
        doLocationSearch(form, config, 'list')
elif action in ['<<', '>>']:
